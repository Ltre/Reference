...
<font id="a15" color="blue">15、用于PV/UV统计的上报接口</font>
    ...
    参数(get|post):
        ...
        scene       业务场景，必选
                    值枚举：
                        'submit_guess',     //提交竞猜时
                        'encour_allot',     //弹出助力分享二维码时
                        'encour_support',   //好友协助点亮时
                        'encour_aff',       //助力分享页内点击召唤好友时
                        'encour_progress'   //助力分享页加载时
        source      来源，可选（此定义比较模糊，当作以后备用的上报字段）
    ————————————————————————————————————————————————————
    返回(object)：
        {
            "rs": true,
    ...
    ————————————————————————————————————————————————————
    示例：
        (new Image).src = 'https://match-guess.duowan.com/visitlog/record?scene=encour_progress&ac_id=26';
    ————————————————————————————————————————————————————
...
<font id="a16" color="blue">16、检测用户是否获奖（竞猜的积分排行上榜等级奖、人工抽取的安慰奖、助力分享的实物奖）</font>
    ————————————————————————————————————————————————————
    URL:
        正式：http://match-guess.duowan.com/user/hasReward
        测试：http://match-guess.webdev2.duowan.com/user/hasReward
    ————————————————————————————————————————————————————
    参数(get|post):
        ac_id       活动ID，必填
    ————————————————————————————————————————————————————
    返回(object)：
        {
            "rs": true, //这个用于判断接口执行是否正常
            "msg": "有奖",
            "hasReward": true, //以这个为准，判断有没有获奖
        }
    ————————————————————————————————————————————————————
    备注: 
        需要登录态
    ————————————————————————————————————————————————————
    示例：
        http://match-guess.duowan.com/user/hasReward?ac_id=26
    ————————————————————————————————————————————————————
