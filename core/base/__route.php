...
final class DIRoute {
    ...
    public function route(){
        DI_ROUTE_REWRITE && $this->rewrite();//重写路由
        ...
        foreach ($rules as $k => $rule) {
            ...
            if (preg_match($k, $req, $matchs)) {
                ...
                foreach($matchs as $matchkey => $matchval){
                    ...
                    //var_dump(compact('matchkey', 'matchval', 'isPosTag', 'keyIsLetter', 'valueIsLetter', 'keyIsWord', 'valueIsWord', 'keyIsNum', 'valueIsNum', 'keyIsNums', 'valueIsNums', 'keyIsVar', 'valueIsVar'));
                    $typeIsNormal = $keyIsLetter && $valueIsLetter || $keyIsWord && $valueIsWord || $keyIsNum && $valueIsNum || $keyIsNums && $valueIsNums || $keyIsVar && $valueIsVar;
                    ...
                    //检查需要匹配类型时，是否匹配成功
                    if ($keyIsLetter&&!$valueIsLetter || $keyIsWord&&!$valueIsWord || $keyIsNum&&!$valueIsNum || $keyIsNums&&!$valueIsNums || $keyIsVar&&!$valueIsVar) {
                ...
                if ($checkTypeInNeed) {
                    ...
                    array_unshift_withkey($GLOBALS['request_args'], $rule, '');
                    return; //到此处为重写成功
                } else {
